...
# REFLEXÃO:
# 1. RFs definem O QUE o sistema faz (validar nome, peso, repetições).
#    RNFs definem COMO ele se comporta (tempo de resposta, segurança).
#    Na prática, RFs viraram ifs de validação; o RNF virou medição de tempo.
#
# 2. Sem o RNF de performance, o sistema funcionaria mas poderia ser lento
#    demais para uso real — ninguém espera 2s para registrar uma série no treino.
#
# 3. RNF não implementado: o sistema deveria exigir autenticação do aluno
#    antes de registrar qualquer dado (segurança / controle de acesso).